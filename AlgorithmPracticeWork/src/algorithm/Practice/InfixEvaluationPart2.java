package algorithm.Practice;

import java.util.StringTokenizer;

public class InfixEvaluationPart2 {
	public static String evaluateInfix(String exps) {

		/** remove if any spaces from the expression **/
		exps = exps.replaceAll("\\s+", "");
		/** we assume that the expression is in valid format **/
		MyGenericsStack<String> stack = new MyGenericsStack<String>(exps.length());
		/** break the expression into tokens **/
		StringTokenizer tokens = new StringTokenizer(exps, "{}()*/+-", true);
		while (tokens.hasMoreTokens()) {
			String tkn = tokens.nextToken();
			/** read each token and take action **/
			if (tkn.equals("(") || tkn.equals("{") || tkn.matches("[0-9]+") || tkn.equals("*") || tkn.equals("/")
					|| tkn.equals("+") || tkn.equals("-")) {
				/** push token to the stack **/
				stack.push(tkn);
			} else if (tkn.equals("}") || tkn.equals(")")) {
				try {
					int op2 = Integer.parseInt(stack.pop());
					String oprnd = stack.pop();
					int op1 = Integer.parseInt(stack.pop());
					/** Below pop removes either } or ) from stack **/
					if (!stack.isStackEmpty()) {
						stack.pop();
					}
					int result = 0;
					if (oprnd.equals("*")) {
						result = op1 * op2;
					} else if (oprnd.equals("/")) {
						result = op1 / op2;
					} else if (oprnd.equals("+")) {
						result = op1 + op2;
					} else if (oprnd.equals("-")) {
						result = op1 - op2;
					}
					/** push the result to the stack **/
					stack.push(result + "");
				} catch (Exception e) {
					e.printStackTrace();
					break;
				}
			}
		}
		String finalResult = "";
		try {
			finalResult = stack.pop();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return finalResult;
	}

	public static void main(String a[]) {
		String expr = "((2*5)+(6/2))";
		String expr2 = "4 + 8 * 6 - 5 / 3 - 2 * 2 + 2";
		String infixExpression = "2 * (5 *(3+6))/15-2";
		String expr3 = "(4+8) * (6-5) / ((3-2) * (2+2))";
//		System.out.println("Expression: " + expr);
//		System.out.println("Expression: " + expr2);
//		System.out.println("Expression: " + infixExpression);
//		System.out.println("------------------------------------");
//		System.out.println("Final Result: infixExpression = " + evaluateInfix(infixExpression));
//		System.out.println("------------------------------------");
//		System.out.println("------------------------------------");
//		System.out.println("Final Result: " + evaluateInfix(expr2));
//		System.out.println("------------------------------------");
//		System.out.println("Final Result: " + evaluateInfix(expr));
//		expr = "(((2 * 5) - (1 * 2)) / (11 - 9))";
//		System.out.println("Expression: " + expr);
//		System.out.println("Final Result: " + evaluateInfix(expr));
		System.out.println("Final Result: " + evaluateInfix(expr2));

	}
}

/**
 * Stack implementation
 */
